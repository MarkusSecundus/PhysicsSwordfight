...
\begin{itemize}
    ...
    \item \textbf{Nízká specifičnost} - Není možné uspokojivě ztvárnit veškerá zajímavá specifika konkrétní zbraně. Všechny zbraně se musejí chovat do jisté míry podobně. Často je tak zbraň smrštěna do pouhé tabulky statů\footnote{Povrchní veličiny typu dosah, rychlost úderu, míra způsobeného zranění, typ zranění (prvek výčtového typu - bodné|sečné|tupé|magické|apod.), které zbraň formálně popisují}.
\end{itemize}
...
Zaklínač 3 \cite{Wiedzmin3} je akční \acs{RPG} hra, která byla vydána r. 2015 polským studiem CD Projekt. Mezi její hlavní důrazy patří otevřený fantasy svět, propracovaný nelineární příběh\footnote{Hra vychází z knižní série Zaklínač Andrzeje Sapkowského \cite{SapkowskiZaklinac}} a v neposlední řadě také dynamický bojový systém.

Hráč se zde ujímá role Geralta z Rivie - zaklínače, profesionálního lovce monster. Ten je kromě svých dvou mečů\footnote{stříbrného pro boj s nestvůrami a železného proti lidem} vybaven flexibilními možnostmi pohybu, jednoduchými kouzly (tzv. zaklínačská znamení) a širokou škálou ručně vařených lektvarů. Na své cestě bojuje s lidmi, upíry, bazilišky i nepředstavitelnými monstry z nočních můr
...
(proč se jí nebudeme pro výzkumu zabývat)

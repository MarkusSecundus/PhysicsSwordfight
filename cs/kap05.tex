...
Řešení je jednoduché - kameru jsme zkrátka umístili v objektové hierarchii mezi děti šermířova herního objektu, napolohovali jsme ji na příhodné místo u jeho očí, a nyní se při šermířově pohybu přesunuje adekvátně spolu s ním. 
...
Nejprve se podívejme odděleně na rotační složku pohybu. Tu bylo možné nechat téměř zcela v režii ConfigurableJointu.

Bylo pouze třeba nakonfigurovat na něm z editoru nenulový angularDrive a všechny rotační stupně volnosti nastavit jako Free. Poté již stačilo pouze přepisovat jeho pole targetRotation a meč se sám silou jointu plynule pohyboval na kýženou pozici.

Této metodě nečinil problém ani fakt, že je cílová pozice každý snímek novým voláním MoveSword() přepsána - každý snímek zkrátka na meč byla aplikována síla posunující jej k té aktuální targetRotation. 

Jediný větší problém, na který jsme zde narazili, je abnormální souřadnicový systém, který ConfigurableJoint pro targetRotation používá - metodu, jak do něj převést rotaci v klasických světových souřadnicích, kterou nám předala MoveSword(), rozebereme později v \ref{howToSetJointsTargetRotationSection}. 

Rovněž bylo třeba vyladit parametry jak na rigidbody meče (těžiště a inertiaTensor), tak nalézt vhodný angularDrive. Následně jsem však získali meč, který je schopen velmi plynulého a celkově pěkného úhlového pohybu a nečiní mu problém ani být realisticky ovlivňován kolizemi s cizími předměty (např. být sražen pod úderem nepřátelského meče) - nad mírou této jeho schopnosti navíc máme vcelku značnou kontrolu skrze konfiguraci angularDrive (čím větší spring, tím více drží v cílové pozici).
...
\begin{itemize}
  ...
  \item \textit{joint.slerpDrive} - spring: 120\footnote{Pro mod blokování spring: 2400 - jinak bylo meč pro protivníka příliš jednoduché srazit na stranu}, damper: 10
\end{itemize}
...
\subsubsection*{Další parametry držení meče}
